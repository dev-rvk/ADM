...
export class StreamClientScrcpy
    ...
    public getDeviceView(): HTMLDivElement {
    ...
    public attachTouchListeners(): void {
        ...
        this.touchHandler = new FeaturedTouchHandler(this.player, this);
    }

    public attachMultipleTouchListeners(...player: TouchHandlerListener[]): void {
        if (player.length <= 0) {
            return;
        }
        if (typeof this.player === 'undefined') {
